...
export default defineConfig({
  testDir: './e2e',
  fullyParallel: true,
  timeout: 120_000, // the axe driver walks every exhibit and every attack preset
   ...
   * constraints:
   *
   *   - `npm run test:a11y` (what deploy.yml's gate step runs) selects only the
   *     Chromium projects, because that workflow installs only Chromium.
   *   - `.github/workflows/e2e.yml` installs all three engines and runs everything,
   *     so Firefox and WebKit regressions are still caught in CI.
   */
  projects: [
    {
      name: 'a11y',
  ...
  webServer: {
    command: 'npm run preview -- --port 4231 --strictPort',
