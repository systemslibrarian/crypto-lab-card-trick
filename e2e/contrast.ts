 ...
 *    `color-mix()`. Here that is the hero aside,
 *    `color-mix(in oklab, var(--accent) 6%, transparent)` over the page
 *    background, the shared top bar (which mixes both its ink and its border out
 *    of `--accent`), and the two `linear-gradient()` layers that draw the select
 *    chevron. Chromium hands `color-mix()` back to `getComputedStyle`
 *    unresolved, so axe files nodes on those surfaces under `incomplete` rather
 *    than measuring them.
 *  - TEXT INSIDE A `role="img"` GRAPHIC. This is the half that matters here:
 *    the advantage-vs-work chart in the Compare exhibit is one `<svg
 *    role="img">` with an `aria-label`, and everything inside it — the axis
 *    ticks at 11px, the `circuit` and `cards` series labels at 12px/700, the
 *    axis title — is a single image as far as the accessibility tree is
 *    concerned, so axe's `color-contrast` rule never looks at any of it. Those
 *    glyphs take their ink from `fill`, not `color`, and the series colours they
 *    use were chosen to separate two LINES (a 3:1 graphics threshold) before
 *    they were reused as small bold TEXT (4.5:1). Nothing but an arithmetic walk
 *    measures them.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab already refuses that
 *    idiom on purpose (`--text-dim` is "muted via lower lightness, never
 *    opacity", and `.cl-hero-sub` explicitly overrides the shared hero's
 *    `opacity: .85` back to `1` with a real colour), so the opacity path is
 *    dormant here — which is a property of this stylesheet today, not a
 *    guarantee, and it is carried so the first `opacity:` on a text element is
 ...
 *    boxes are the `.table-wrap` scrollers `dom.ts`'s `scrollRegion` builds
 *    around the wide tables — the indistinguishability table, the dealer
 *    distribution, the comparison grid. Content scrolled past them is
 ...
 *    has real SVG text — the chart's ticks, series labels and axis title — and
 *    the shapes drawn before them are the gridlines, axes, cursor and the two
 *    series polylines. An ancestor-only walk would composite each `<text>` onto
 *    the `<svg>`'s own transparent background and then onto the panel, which is
 *    the right answer here only because none of those shapes paints a fill
 *    behind a label; the sibling walk is what establishes that rather than
 *    assuming it.
 */
 ...
 * uses. Nothing here is transient: the stylesheet's animations are all
 * entry effects (`reveal`, `dealin`, `flipin`) that its reduced-motion block
 * switches off, so under the preference the gate asks for, every state the drive
 * builds simply stays on screen until the next click. The parameter is
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven scan here walks a ten-row orbit enumeration, a full
     * indistinguishability table, a five-row dealer distribution with its
     * sliders, the comparison grid, the chart's ticks and labels and every
     * learner check on the open tab, and every one of those nodes re-walks the
     * same handful of
     ...
     * those are the `.table-wrap` scrollers around the wide tables.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This lab's chart is exactly that shape: five
      // `.chart-grid` rules, two `.chart-axis` lines and a `.chart-cursor` all
      // cross the tick labels, and every one of them is a stroke-only `<line>`
      // whose computed `fill` is the initial black. Excluding unfilled tags is
      // what keeps the tick labels measured against the panel they actually sit
      // on. The two series `<polyline>`s are skipped by the same list, and the
      // `<circle>` markers survive it but are 10px across, so `contains` rejects
      // them against any label box.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. `checkVisibility()` catches it. This lab's case is
      // `dom.ts`'s `disclosure()` blocks, shut on arrival across all five
      // exhibits, which the gate opens by clicking their `<summary>` — the route
      // a reader has — rather than setting `.open` from script, which is what
      // the gate this replaces did to every one of them at once. It also catches
      // the four `hidden` tab panels: only the selected exhibit is in the
      // accessibility tree and only the selected exhibit is measured, so the
      // drive switches tabs rather than un-hiding them.
      ...
      // The VISUALLY-HIDDEN idiom paints no pixels either, and it does not look
      // like any of the cases above: `.sr-only` here is `position: absolute;
      // width: 1px; height: 1px; overflow: hidden; clip-path: inset(50%)`, so it
      // has a non-zero rect, a non-zero opacity, passes `checkVisibility()` and
      // sits at a real document position. Every glyph in it is nevertheless
      // clipped away — that is the entire point of the class, and this page uses
      // it heavily: every card carries `srOnly('spade')` beside its pip so the
      // suit is never conveyed by shape alone, and every table has an `.sr-only`
      // caption. Measuring that text reports ratios for ink that is never laid
      // down, and it reports them against whatever surface the hidden span
      // happens to sit on: the spade/heart labels inside a face-up card came out
      // at 1.15:1 and the "face down" label inside a card back at 1.66:1, for
      // text no sighted reader has ever seen. axe's own `color-contrast` rule
      // excludes the same nodes for the same reason, so skipping them keeps this
      // oracle aligned with it rather than stricter in a direction that has no
      // meaning. Both spellings of the idiom are covered, plus the older
      // `clip: rect(0 0 0 0)` form.
      if (cs.clip === 'rect(0px, 0px, 0px, 0px)') return false;
      if (/inset\(\s*(?:50%|100%)/.test(cs.clipPath)) return false;
      if ((r.width <= 1 || r.height <= 1) && /hidden|clip/.test(cs.overflowX + cs.overflowY))
        return false;
      return true;
    };
     ...
     * hamburger and GitHub marks, the `.cut-arrow` glyph between the two halves
     * of the cut figure, the suit pips repeated beside their spelled-out names —
     * would be
     ...
     * attribute. `role="img"` is deliberately NOT treated the same way: the
     * chart's glyphs are painted text a sighted reader reads, and skipping them
     * is what left them unmeasured by every oracle before this one.
     */
     ...
     *
     * This lab draws real SVG — the chart, and the card faces `ui/cards.ts`
     * builds — through `dom.ts`'s `svg()` helper, whose `Child` type accepts
     * strings, so a caller can put character data into a `<g>` or a `<circle>`
     * as easily as into a `<text>`. That text is invisible on screen, but a walk
     * that measures it reports a 1:1 failure against the panel — a ratio
